    ...
    <div class="card-body pd-25">
        <div class="table-wrapper">
            <table id="datatable1" class="table display responsive nowrap">
                ...
                <tbody> 
                     @foreach ($ticketForwardList as $forward)
                        <tr>
                            <td>{{$forward->id}}</td>
                            <td>{{$forward->ticket_id}}</td>
                            <td>{{User::List($forward->accepted_by)->name}}</td>
                            <td>{{User::List($forward->forwarded_to)->name}}</td>
                            <td>{{$forward->reason}}</td>
                            <td>{{$forward->forwarded_date}}</td>
                        </tr> 
                    @endforeach 
                </tbody>
            </table>
            {{ $ticketForwardList->links() }}
        </div><!-- table-wrapper -->
    </div>
    <!-- card-body -->
</div>
